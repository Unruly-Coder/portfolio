...
export class Bubbles {
    ...
    sides.forEach((side) => {
      verticalAngles.forEach((verticalAngle) => {
        horizontalAngles.forEach((horizontalAngle) => {
          ...
          bubbleEmitter.instance.position.setFromSphericalCoords(
            this.submarine.submarineRadius + 0.25,
            Math.PI * verticalAngle,
            Math.PI *
              (side === "right" ? horizontalAngle : horizontalAngle + 1),
          );
  ...
  update() {
    this.bubbleEmitters.forEach((bubbleEmitter) => {
      if (this.isBubbling) {
        if (
          this.submarine.direction.dot(bubbleEmitter.getEmitterDirection()) >
          0.05
        ) {
